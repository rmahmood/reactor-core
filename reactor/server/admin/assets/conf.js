...
function makeConfig(context) {
    ...
    // Construct config submit button.
    container_elt.append(
        $("<a>Save</a>")
            .attr({
                "id": "config-submit",
...
function readConfig(template, sub_templates, toplevel_sections) {
...
function postConfig(context, post_url) {
    $.ajax({
        ...
        type: "POST",
        dataType: "json",
        data: JSON.stringify(readConfig(context["template"],
                                        context["sub_templates"],
                                        context["toplevel_sections"])),
        processData: false,
...
function generateTemplate(root, name, template) {
    ...
    $.each(template, function(config_name, config) {
        generateSingleConfig(fs, config_name, config);
    });
}
...
function generateSingleConfig(root, config_name, config)
{
    ...
    function buildConfigSlot(root, name) {
        var cgroup = $("<div/>").attr("class", "control-group").appendTo(root)
            .append($("<label/>").attr("class", "control-label").html(name));
    ...
    case "boolean":
        input_elt = $("<input/>")
            .attr("type", "checkbox")
            .prop("checked", parseBool(config["value"] || "false"))
            .appendTo(
